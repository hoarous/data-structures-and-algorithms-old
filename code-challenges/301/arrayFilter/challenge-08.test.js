...
const filterStringsWithVowels = (arr) => {
  // Solution code here...
  return arr.filter(n => n.search(/[aeiou]/ig)+1);
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  ...
  return arr.filter(function(n){
    let i = 0;
    let found = false;
    while(!found && i < forbiddenValues.length){
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  return getBaseStatGreaterThan(arr, minBaseStat).map(n => n.stat.name);
};
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  return arr.filter(n => Number.isInteger(n)).map(n => {
      switch(n%2){
        case 0: return 'even';
        case 1: return 'odd';
      }
