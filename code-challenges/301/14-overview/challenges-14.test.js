...
const toTitleCase = (arr) => {
  // Solution code here...
  return arr.map((val) => val.charAt(0).toUpperCase() + val.substring(1));
};
...
let biggerThanLuke = (arr) => {
  ...
  return arr.filter((val) => {
    return Number.parseInt(val.mass) > Number.parseInt(starWarsData[0].mass)}).map((character) => {
      return character.name}).join(' - ');
}
...
const detectTicTacToeWin = (board) => {
  // Solution code here...
  for(let i = 0; i < 3; i++){
    if(checkCoords(board,i,i,i,0,1,2)) return true;
    else if (checkCoords(board,0,1,2,i,i,i)) return true;
  }
  if(checkCoords(board,0,1,2,0,1,2)) return true;
  else if(checkCoords(board,0,1,2,2,1,0)) return true;
  else return false;
}

const checkCoords = (board, r1, r2, r3, c1, c2, c3) => {
