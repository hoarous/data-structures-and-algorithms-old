...
const onlyOddChars = (str) => {
  // Solution code here...
  return str.split('').filter((val, i) => i%2).join('');
};
...
const allHappy = (arr) => {
  // Solution code here...
  return arr.length === findHappiness(arr).length;
};
...
const findEvery = (arr, target) => {
  // Solution code here...
  let i = 0;
  while(i < arr.length && arr[i].includes(target)) i++;
  return i === arr.length;
};
...
const unenrollBrook = (arr) => {
  // Solution code here...
  return arr.map((val) => val.filter((name) => !name.includes('Brook')));
};
...
const sortByDay = (arr) => {
  ...
  for(let i = 0; i < daysOfWeek.length; i++){
    schedule.push(findAnything(arr, daysOfWeek[i]));
  }
  return schedule;
};
...
const characterByIndex = (arr) => {
  // Solution code here...
  return arr.map((val, i) => val.substring(i, i+1));
};
