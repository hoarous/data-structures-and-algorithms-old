...
const isCapitalized = (str) => {
  // Solution code here...
  return str.toString().match(/\b[A-Z]\w*/g);
};
...
const citiesAtoJ = (arr) => {
  ...
  for(let i=0; i < arr.length; i++){
    let match = arr[i].match(/^[A-J].*/);
    if(match){
        returnArr.push(match[0]);
    }
  }
  return returnArr;
};
...
const matchMonth = (input) => {
    return input.toString().match(/^(o|O)ct(ober)?$/);
};
...
const noPunctuation = str => {
  return str.toString().match(/\b\w*\s/g);
};
