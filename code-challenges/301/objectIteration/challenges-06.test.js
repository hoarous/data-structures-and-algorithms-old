...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  let i = 0;
  while(Object.values(arr[i])[0]!==character && i < arr.length){
      i++
  }
  return Object.values(arr[i])[2].length;
}
...
const hasChildrenEntries = (arr, character) => {
  // Solution code here...
  let i = 0;
  while(Object.entries(arr[i])[0][1]!==character && i < arr.length){
      i++
  }
  return Object.entries(arr[i])[2][1].length;
}
...
const houseSize = (arr) => {
  ...
  arr.forEach(function(element){
    let family = Object.values(element);
    let entry = {house: family[3], members: 1 + family[2].length};
    if(family[1]){
        entry.members++;
    }
    sizes.push(entry);
...
const houseSurvivors = (arr) => {
  ...
  arr.forEach(function(element){
    let family = Object.values(element);
    let entry = {house: family[3], members: 1 + family[2].length};
    if(family[1] && !(deceasedSpouses.indexOf(family[1])+1)){
        entry.members++;
    }
