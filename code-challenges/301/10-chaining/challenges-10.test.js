...
const count = (target, input) => {
  // Solution code here...
  return input.reduce((acc1, arr) => {
    acc1 += arr.reduce((acc2, val) => {
      if(val === target) acc2++;
      return acc2;
    }, 0);
    return acc1;
...
const totalSum = (input) => {
  // Solution code here...
  return input.reduce((acc1, arr) => {
      return acc1 + arr.reduce((acc2, val) =>{
        return acc2 +val;
      }, 0);
  }, 0);
};
...
const divisibleByFiveTwoToThePower = (input) => {
  // Solution code here...
  return input.map((arr)=> arr.filter((val) => {
...
let findShortest = (data) => {
  ...
  return data.reduce((acc,val) => {
    if(acc === null || parseInt(acc.height) > parseInt(val.height)) return val;
    else return acc;
  }, null).name;
}
