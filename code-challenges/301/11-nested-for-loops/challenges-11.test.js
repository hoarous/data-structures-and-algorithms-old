...
const validatePin = (pin) => {
  // Solution code here...
  return pin.toString().match(/^[0-9][0-9][0-9][0-9]$/)
};
...
const validateEmail = (email) => {
  // Solution code here...
  return email.toString().match(/^\w+(\.\w+)?@\w+.((com)|(net)|(org))$/);
};
...
const validatePhoneNumber = (phoneNumber) => {
  // Solution code here...
  return phoneNumber.toString().match(/^((\([0-9]{3}\))|([0-9]{3}))( |-)?[0-9]{3}( |-)?[0-9]{4}$/);
};
...
const findTagNames = elements => {
  // Solution code here...
  return elements.toString().match(/\/\w+(?=>)/g);
}
