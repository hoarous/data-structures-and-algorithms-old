...
const charCode = (arr) => {
  // Solution code here...
  return arr.map(n => n.charCodeAt(0));
};
...
const extractAbilities = (arr) => {
  // Solution code here...
  return arr.map(n => n.ability.name);
};
...
const extractStats = (arr) => {
  ...
  return arr.map(function(n){
    let skillName = n.stat.name;
    let skillVal = n.effort + n.baseStat;
    return {name:skillName,total:skillVal};
  });
};
